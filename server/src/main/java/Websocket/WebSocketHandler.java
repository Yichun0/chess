...
//      username match the white username
    ...
    public void makeMove(Session session, String message) throws DataAccessException, IOException, SQLException {
        MakeMove makeMoves = new Gson().fromJson(message, MakeMove.class);
...
//                if (currentGame.isInCheckmate(ChessGame.TeamColor.BLACK)){
//                    String inCheckMessage = game.getBlackUsername() + " is in check. The winner is " + game.getWhiteUsername();
//                    connectionManager.notifyEveryUser(gameID,inCheckMessage);
//                } else if (currentGame.isInStalemate(ChessGame.TeamColor.BLACK)) {
//                    String inStalemate = game.getWhiteUsername() + " is in stalemate. This is a tie.";
//                    connectionManager.notifyEveryUser(gameID,inStalemate);
//                }
            } else if (!isNull(game.getWhiteUsername())) {
                if (game.getWhiteUsername().equalsIgnoreCase(username)) {
